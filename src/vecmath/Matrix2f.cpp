...
Matrix2f::Matrix2f(const Matrix2f& rm) { memcpy(m_elements, rm.m_elements, 4 * sizeof(double)); }
...
const double& Matrix2f::operator()(int i, int j) const { return m_elements[j * 2 + i]; }

double& Matrix2f::operator()(int i, int j) { return m_elements[j * 2 + i]; }
...
double Matrix2f::determinant() { return Matrix2f::determinant2x2(m_elements[0], m_elements[2], m_elements[1], m_elements[3]); }
...
Matrix2f Matrix2f::inverse(bool* pbIsSingular, double epsilon) {
    ...
    if (isSingular) {
        ...
        return Matrix2f(m_elements[3] * reciprocalDeterminant, -m_elements[2] * reciprocalDeterminant, -m_elements[1] * reciprocalDeterminant, m_elements[0] * reciprocalDeterminant);
    }
}
...
Matrix2f::operator double*() { return m_elements; }

void Matrix2f::print() const { printf("[ %.4f %.4f ]\n[ %.4f %.4f ]\n", m_elements[0], m_elements[2], m_elements[1], m_elements[3]); }

// static
double Matrix2f::determinant2x2(double m00, double m01, double m10, double m11) { return (m00 * m11 - m01 * m10); }
...
Matrix2f operator*(const Matrix2f& m, double f) { return f * m; }
