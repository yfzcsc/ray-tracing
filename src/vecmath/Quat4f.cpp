...
const double& Quat4f::operator[](int i) const { return m_elements[i]; }

double& Quat4f::operator[](int i) { return m_elements[i]; }

double Quat4f::w() const { return m_elements[0]; }

double Quat4f::x() const { return m_elements[1]; }

double Quat4f::y() const { return m_elements[2]; }

double Quat4f::z() const { return m_elements[3]; }

Vector3f Quat4f::xyz() const { return Vector3f(m_elements[1], m_elements[2], m_elements[3]); }

Vector4f Quat4f::wxyz() const { return Vector4f(m_elements[0], m_elements[1], m_elements[2], m_elements[3]); }

double Quat4f::abs() const { return sqrt(absSquared()); }

double Quat4f::absSquared() const { return (m_elements[0] * m_elements[0] + m_elements[1] * m_elements[1] + m_elements[2] * m_elements[2] + m_elements[3] * m_elements[3]); }
...
Quat4f Quat4f::conjugated() const { return Quat4f(m_elements[0], -m_elements[1], -m_elements[2], -m_elements[3]); }
...
Quat4f Quat4f::log() const {
    double len = sqrt(m_elements[1] * m_elements[1] + m_elements[2] * m_elements[2] + m_elements[3] * m_elements[3]);
...
Quat4f Quat4f::exp() const {
    double theta = sqrt(m_elements[1] * m_elements[1] + m_elements[2] * m_elements[2] + m_elements[3] * m_elements[3]);
...
Vector3f Quat4f::getAxisAngle(double* radiansOut) {
    ...
    *radiansOut = theta;
    return Vector3f(x() * reciprocalVectorNorm, y() * reciprocalVectorNorm, z() * reciprocalVectorNorm);
}
...
void Quat4f::print() { printf("< %.4f + %.4f i + %.4f j + %.4f k >\n", m_elements[0], m_elements[1], m_elements[2], m_elements[3]); }

// static
double Quat4f::dot(const Quat4f& q0, const Quat4f& q1) { return (q0.w() * q1.w() + q0.x() * q1.x() + q0.y() * q1.y() + q0.z() * q1.z()); }

// static
Quat4f Quat4f::lerp(const Quat4f& q0, const Quat4f& q1, double alpha) { return ((q0 + alpha * (q1 - q0)).normalized()); }
...
// static
Quat4f Quat4f::fromRotatedBasis(const Vector3f& x, const Vector3f& y, const Vector3f& z) { return fromRotationMatrix(Matrix3f(x, y, z)); }
...
Quat4f operator+(const Quat4f& q0, const Quat4f& q1) { return Quat4f(q0.w() + q1.w(), q0.x() + q1.x(), q0.y() + q1.y(), q0.z() + q1.z()); }

Quat4f operator-(const Quat4f& q0, const Quat4f& q1) { return Quat4f(q0.w() - q1.w(), q0.x() - q1.x(), q0.y() - q1.y(), q0.z() - q1.z()); }

Quat4f operator*(const Quat4f& q0, const Quat4f& q1) { return Quat4f(q0.w() * q1.w() - q0.x() * q1.x() - q0.y() * q1.y() - q0.z() * q1.z(), q0.w() * q1.x() + q0.x() * q1.w() + q0.y() * q1.z() - q0.z() * q1.y(), q0.w() * q1.y() - q0.x() * q1.z() + q0.y() * q1.w() + q0.z() * q1.x(), q0.w() * q1.z() + q0.x() * q1.y() - q0.y() * q1.x() + q0.z() * q1.w()); }

Quat4f operator*(double f, const Quat4f& q) { return Quat4f(f * q.w(), f * q.x(), f * q.y(), f * q.z()); }

Quat4f operator*(const Quat4f& q, double f) { return Quat4f(f * q.w(), f * q.x(), f * q.y(), f * q.z()); }
